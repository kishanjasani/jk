...
function jk_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'jk' ),
...
function jk_excerpt_more( $more = '' ) {
	if ( ! is_single() ) {
		$more = sprintf(
			'<a class="jk-read-more text-white" href="%1$s"><button class="mt-4 btn btn-info">%2$s</button></a>',
...
function jk_pagination() {
	...
	printf( '<nav class="jk-pagination clearfix">%s</nav>', wp_kses( paginate_links( $args ), $allowed_tags ) );
}
